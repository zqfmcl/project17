...
<html>
  <head>
    ...
    <title>My JSP 'nry_main.jsp' starting page</title>
...
<%
	...
	String feny="<td style='color:blue;' onclick='javascript:location.href=\"Nry.do?method=feny&toPage="+pgh.getNowPage()+"\"'>"+pgh.getNowPage()+"</td>";
	if(pgh.getNowPage()>1){
	feny="<td onclick='javascript:location.href=\"Nry.do?method=feny&toPage="+(pgh.getNowPage()-1)+"\"'>"+(pgh.getNowPage()-1)+"</td>"+feny;
	}
	if(pgh.getNowPage()>2){
	feny="<td onclick='javascript:location.href=\"Nry.do?method=feny&toPage="+(pgh.getNowPage()-2)+"\"'>"+(pgh.getNowPage()-2)+"</td>"+feny;
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=1){
	feny+="<td onclick='javascript:location.href=\"Nry.do?method=feny&toPage="+(pgh.getNowPage()+1)+"\"'>"+(pgh.getNowPage()+1)+"</td>";
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=2){
	feny+="<td onclick='javascript:location.href=\"Nry.do?method=feny&toPage="+(pgh.getNowPage()+2)+"\"'>"+(pgh.getNowPage()+2)+"</td>";
	}
	...
	for(int i=0;i<list.size();i++){
		...
		table+="<tr>";
		table+="<td>"+rec.get("BT")+"</td>";
		table+="<td>"+rec.get("FBSJ")+"</td>";
		table+="<td>"+rec.get("LMMC")+"</td>";
		table+="<td>"+rec.get("YDCS")+"</td>";
		table+="<td>"+rec.get("SFXS")+"</td>";
		table+="<td><a  href='Nry.do?method=show&xmid="+rec.get("ID")+"'>查看</a> "+
		"<a  href='JSP/xwgl/nry_edit.jsp?xmid="+rec.get("ID")+"'>编辑</a> "+
...
background: #F5F5F5;
  ...
  <body>
  <div class="title"><span class="tcontext">新闻内容</span></div>
  <form method="post" action="Nry.do?method=select">
    ...
    <tr>
    <td colspan="6">
    	<input type="button"  value="添加" class="btn" onclick="javascript:location.href='JSP/xwgl/nry_add.jsp'"/>  
    </tr>
    </table>
  </form>
  <div class="tlist">
    ...
    <div class="mutiTable"
			...
			<table id="table1" cellpadding="1px" border="0" cellspacing="1"
				...
				<tr>
					<th rowspan="1" style="width:12%">标题</th>
					<th rowspan="1" style="width:14%">发布时间</th>
					<th rowspan="1" style="width:14%">所属栏目</th>
					<th rowspan="1" style="width:10%">阅读次数</th>
					<th rowspan="1" style="width:10%">首页显示</th>
					<th rowspan="1" style="width:12%">操作</th>
				</tr>
				<%=table %>
  			</table></div>
    	...
    	<div style="margin-top: 10px;">
    		...
    		<table class="changePage">
    			<tr><td style="background-image: url('Image/upPage.gif');" onclick="<%=(pgh.getNowPage()>1?"javascript:location.href='Nry.do?method=feny&toPage=sub'":"") %>"></td>
    			<%=feny %>
    			<td style="background-image: url('Image/downPage.gif');" onclick="<%=(pgh.getSumPage()-pgh.getNowPage()>=1?"javascript:location.href='Nry.do?method=feny&toPage=add'":"") %>"></td>
    			<td  style="width: 88px;">
  ...
  function dodel(id){
  	if(window.confirm("是否确认删除？")){
    			url="Nry.do?method=del&xmid="+id;
    			window.location.href=url;
    }
  }
  function feny(sel){
  location.href="Nry.do?method=chgPage&pNum="+sel.options[sel.selectedIndex].value;
  }
function suitHeight(){
	var bgc=true;
