...
<html>
  ...
  <%
	...
	String feny="<td style='color:blue;' onclick='javascript:location.href=\"Xmddsb.do?method=feny1&toPage="+pgh.getNowPage()+"\"'>"+pgh.getNowPage()+"</td>";
	if(pgh.getNowPage()>1){
	feny="<td onclick='javascript:location.href=\"Xmddsb.do?method=feny1&toPage="+(pgh.getNowPage()-1)+"\"'>"+(pgh.getNowPage()-1)+"</td>"+feny;
	}
	if(pgh.getNowPage()>2){
	feny="<td onclick='javascript:location.href=\"Xmddsb.do?method=feny1&toPage="+(pgh.getNowPage()-2)+"\"'>"+(pgh.getNowPage()-2)+"</td>"+feny;
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=1){
	feny+="<td onclick='javascript:location.href=\"Xmddsb.do?method=feny1&toPage="+(pgh.getNowPage()+1)+"\"'>"+(pgh.getNowPage()+1)+"</td>";
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=2){
	feny+="<td onclick='javascript:location.href=\"Xmddsb.do?method=feny1&toPage="+(pgh.getNowPage()+2)+"\"'>"+(pgh.getNowPage()+2)+"</td>";
	}
	...
	for(int i=0;i<list.size();i++){
		...
		tableR+="<td><a data-num-p='0603' href='Xmddsb.do?method=show&xmid="+rec.get("ID")+"'>查看</a> "+
		//"<a data-num-p='0605' href='javascript:dodel("+rec.get("ID")+")'>删除</a> "+
		"<a data-num-p='0605' onclick=\"foujue("+rec.get("ID")+","+rec.get("XM")+")\">否决</a> "+
		"<a data-num-p='0607' onclick=\"huitui("+rec.get("ID")+","+rec.get("XM")+")\">回退</a></td>";
		tableL+="</tr>";
		tableR+="</tr>";
	}
...
.tlist td{ text-align:center;}
  ...
  <body>
  <div class="title"><span class="tcontext">项目调度管理(已审核)</span></div>
  <form method="post" action="Xmddsb.do?method=select1">
    ...
    <td colspan="6">
    	...
    	<input data-num-p='0601' type="button" value="添加" class="btn" onclick="javascript:location.href='JSP/xmddgl/xmddglsb_add.jsp'"/>  
    </tr>
    </table>
  </form>
  <div class="tlist">
    <div class="mutiTable" style="margin: 0px;padding: 0px;">
    	...
    	<div style="float: left;">
    		<table class="changePage">
    			<tr><td style="background-image: url('Image/upPage.gif');" onclick="<%=(pgh.getNowPage()>1?"javascript:location.href='Xmddsb.do?method=feny1&toPage=sub'":"") %>"></td>
    			<%=feny %>
    			<td style="background-image: url('Image/downPage.gif');" onclick="<%=(pgh.getSumPage()-pgh.getNowPage()>=1?"javascript:location.href='Xmddsb.do?method=feny1&toPage=add'":"") %>"></td>
    			<td  style="width: 88px;">
  	...
  	$("#reason").val('');
  			...
  			$("#fjqr").click(function(){
				...
				reason=reason.replace(reg,'<br>');
  				url="Xmddsb.do?method=fouj1&reason="+reason+"&xmid2="+xmid2+"&ddid="+ddid;
  ...
  function huitui(ddid,xmid2){
  	$("#reason").val('');
  			...
  			$("#fjqr").click(function(){
  				...
  				var reg=new RegExp("\n","g");
				reason=reason.replace(reg,'<br>');
  				url="Xmddsb.do?method=huit1&reason="+reason+"&xmid2="+xmid2+"&ddid="+ddid;
    			window.location.href=url;
  				
  			});
  } 
  manageView("<%=request.getSession().getAttribute("power").toString()%>","06");
  function dodel(id){
  ...
  function feny(sel){
  location.href="Xmddsb.do?method=chgPage1&pNum="+sel.options[sel.selectedIndex].value;
  }
function suitHeight(){
	var bgc=true;
