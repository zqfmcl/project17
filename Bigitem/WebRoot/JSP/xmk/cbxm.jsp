...
<%
	...
	String feny="<td style='color:blue;' onclick='javascript:location.href=\"Xmsb.do?method=fenycb&toPage="+pgh.getNowPage()+"\"'>"+pgh.getNowPage()+"</td>";
	if(pgh.getNowPage()>1){
	feny="<td onclick='javascript:location.href=\"Xmsb.do?method=fenycb&toPage="+(pgh.getNowPage()-1)+"\"'>"+(pgh.getNowPage()-1)+"</td>"+feny;
	}
	if(pgh.getNowPage()>2){
	feny="<td onclick='javascript:location.href=\"Xmsb.do?method=fenycb&toPage="+(pgh.getNowPage()-2)+"\"'>"+(pgh.getNowPage()-2)+"</td>"+feny;
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=1){
	feny+="<td onclick='javascript:location.href=\"Xmsb.do?method=fenycb&toPage="+(pgh.getNowPage()+1)+"\"'>"+(pgh.getNowPage()+1)+"</td>";
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=2){
	feny+="<td onclick='javascript:location.href=\"Xmsb.do?method=fenycb&toPage="+(pgh.getNowPage()+2)+"\"'>"+(pgh.getNowPage()+2)+"</td>";
	}
	...
	for(int i=0;i<list.size();i++){
		...
		tableR+="<td><a data-num-p='0302' href='Xmsb.do?method=show&xmid="+rec.get("ID")+"'>查看</a> "+
		"<a data-num-p='0306' onclick=\"huitui("+rec.get("ID")+")\" >回退</a></td>";
		tableL+="</tr>";
		tableR+="</tr>";
	}
...
.tlist td{ text-align:center;}
  ...
  <body>
  <div class="title"><span class="tcontext">储备项目</span></div>
  <form method="post" action="Xmk.do?method=cbxm">
    <table class="condition">
    <tr>
    <td style="width:90px">项目年份：</td><td style=""><input type="text" name="q_nf" id="q_nf" style="width:120px"/></td>
    <td style="width:90px;padding-right: 0;"> 重要级别：</td>
    <td style="text-align: left;width:250px;padding: 0;">
    ...
    <td style="width:90px">项目名称：</td><td style="width:230;"><input type="text" name="q_xmmc" id="q_xmmc" style="width:100%"/></td>
    </tr>
    <tr>
    ...
    <td colspan="6">
    	...
    	<input data-num-p="0301" type="button" value="导出" class="btn" onclick="javascript:location.href='Xmsb.do?method=exlExp'"/>
    ...
    <div class="mutiTable" style="max-height:440px;margin: 0px;padding: 0px;">
    	...
    	<div style="float: left;">
    		<table class="changePage">
    			<tr><td style="background-image: url('Image/upPage.gif');" onclick="<%=(pgh.getNowPage()>1?"javascript:location.href='Xmsb.do?method=fenycb&toPage=sub'":"") %>"></td>
    			<%=feny %>
    			<td style="background-image: url('Image/downPage.gif');" onclick="<%=(pgh.getSumPage()-pgh.getNowPage()>=1?"javascript:location.href='Xmsb.do?method=fenycb&toPage=add'":"") %>"></td>
    			<td  style="width: 88px;">
  	...
  	$("#reason").val('');
  			...
  			$("#fjqr").click(function(){
				...
				reason=reason.replace(reg,'<br>');
  				url="Xmk.do?method=cbhuit&reason="+reason+"&xmid="+xmid;
    			window.location.href=url;
  				
  			});
  }
  manageView("<%=request.getSession().getAttribute("power").toString()%>","03");
  
  function feny(sel){
  location.href="Xmsb.do?method=chgPagecb&pNum="+sel.options[sel.selectedIndex].value;
  }
function suitHeight(){
	var bgc=true;
