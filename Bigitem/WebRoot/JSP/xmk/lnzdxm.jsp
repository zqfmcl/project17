...
<%
	...
	String feny="<td style='color:blue;' onclick='javascript:location.href=\"Xmsb.do?method=fenyln&toPage="+pgh.getNowPage()+"\"'>"+pgh.getNowPage()+"</td>";
	if(pgh.getNowPage()>1){
	feny="<td onclick='javascript:location.href=\"Xmsb.do?method=fenyln&toPage="+(pgh.getNowPage()-1)+"\"'>"+(pgh.getNowPage()-1)+"</td>"+feny;
	}
	if(pgh.getNowPage()>2){
	feny="<td onclick='javascript:location.href=\"Xmsb.do?method=fenyln&toPage="+(pgh.getNowPage()-2)+"\"'>"+(pgh.getNowPage()-2)+"</td>"+feny;
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=1){
	feny+="<td onclick='javascript:location.href=\"Xmsb.do?method=fenyln&toPage="+(pgh.getNowPage()+1)+"\"'>"+(pgh.getNowPage()+1)+"</td>";
	}
	if(pgh.getSumPage()-pgh.getNowPage()>=2){
	feny+="<td onclick='javascript:location.href=\"Xmsb.do?method=fenyln&toPage="+(pgh.getNowPage()+2)+"\"'>"+(pgh.getNowPage()+2)+"</td>";
	}
	...
	for(int i=0;i<list.size();i++){
		...
		tableR+="<td><a data-num-p='0402' href='Xmsb.do?method=show&xmid="+rec.get("ID")+"'>查看</a> "+
		"<a data-num-p='0406' onclick=\"huitui("+rec.get("ID")+")\">回退</a></td>";
		tableL+="</tr>";
		tableR+="</tr>";
	}
...
.tlist td{ text-align:center;}
  ...
  <body>
  <div class="title"><span class="tcontext">历年重大项目</span></div>
  <form method="post" action="Xmk.do?method=lnxm">
    <table class="condition">
    <tr>
    <td style="width:90px">项目年份：</td><td style="width:20%"><input type="text" name="q_nf" id="q_nf" style="width:120px"/></td>
    <td style="width:90px;padding-right: 0;">重要级别：</td>
    <td style="width:250px;text-align: left;padding:0;">
    ...
    <td style="width:90px">项目名称：</td><td style="width:230px"><input type="text" name="q_xmmc" id="q_xmmc" style="width:100%"/></td>
    </tr>
    <tr>
    <td style="width:90px">建设阶段：</td><td style=""><select name="q_jsjd" style="">
    			<option value="">请选择</option>
    			<option value="1">新开工</option>
    			<option value="2">续建</option>
    			</select>
    </td><td colspan="4" style="width:70%">所在镇办：&nbsp; <select name="q_szqx" style="">
    			<%=option_szqx %>
    			</select></td>
    ...
    <td colspan="6">
    	...
    	<input data-num-p='0401' type="button" value="导出" class="btn" onclick="javascript:location.href='Xmsb.do?method=exlExp'"/>
  ...
  <div class="tlist">
    <div class="mutiTable" style="margin: 0px;padding: 0px;">
  		<div class="header" style="margin: 0px;padding: 0px;">
  			...
  			<div class="header-right" style="width:633px;float: left;">
    	...
    	<div>
    		<table class="changePage">
    			<tr><td style="background-image: url('Image/upPage.gif');" onclick="<%=(pgh.getNowPage()>1?"javascript:location.href='Xmsb.do?method=fenyln&toPage=sub'":"") %>"></td>
    			<%=feny %>
    			<td style="background-image: url('Image/downPage.gif');" onclick="<%=(pgh.getSumPage()-pgh.getNowPage()>=1?"javascript:location.href='Xmsb.do?method=fenyln&toPage=add'":"") %>"></td>
    			<td  style="width: 88px;">
  	...
  	$("#reason").val('');
  			...
  			$("#fjqr").click(function(){
				...
				reason=reason.replace(reg,'<br>');
  				...
  				url="Xmk.do?method=lnhuit&reason="+reason+"&xmid="+xmid;
    			window.location.href=url;
  				
  			});
  } 
  manageView("<%=request.getSession().getAttribute("power").toString()%>","04");
  
  function feny(sel){
  location.href="Xmsb.do?method=chgPageln&pNum="+sel.options[sel.selectedIndex].value;
  }
function suitHeight(){
	var bgc=true;
