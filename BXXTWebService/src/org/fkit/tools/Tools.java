...
public class Tools {
	...
	public Map<String, Object> queryResultToMap(QueryResult result) {
		...
		if (null != result) {
			...
			LbMetaData metaData = result.getMetaData();
			List<LbRecord> records = result.getRecords();
			if (null != metaData && null != records) {
	...
	public WorkCondition jsonToWorkCondition(JSONObject json) {
		if(null!=json){
			WorkCondition  wc=new WorkCondition();
			wc.setBeginTime(String.valueOf(json.getString("beginTime")));
			wc.setDateRange(String.valueOf(json.getString("dateRange")));
			wc.setEndTime(String.valueOf(json.getString("dateRange")));
			wc.setIncEntrust(Boolean.valueOf(json.getString("incEntrust")));
			wc.setStatus(Integer.parseInt("".equals(json.getString("status"))?"0":json.getString("status")));
			wc.setSubject(String.valueOf(json.getString("subject")));
			wc.setType(String.valueOf(json.getString("type")));
			wc.setWorkName(String.valueOf(json.getString("workName")));
			return wc;
		}else{
			return null;
		}
