...
(function() {
	...
	var g = {
		...
		ua : (function() {
			...
			n = window.opera && window.opera.buildNumber;return {
		...
		cleanName : function(n) {
			...
			n = n.replace(/\s+/g, "_");
			n = n.replace(/[^a-z0-9_\-\.]+/gi, "");return n
		},
		addRuntime : function(n, o) {
			o.name = n;
			k[n] = o;k.push(o);return o
		},
		guid : function() {
	...
	g.Uploader = function(r) {
		...
		function s() {
			...
			if (this.state == g.STARTED) {
				...
				if (x == t.length) {
					this.stop();this.trigger("UploadComplete", t)
				}
			}
		}
		function n() {
		...
		g.extend(this, {
			...
			init : function() {
				...
				r.max_file_size = g.parseSize(r.max_file_size);A.bind("FilesAdded", function(C, F) {
					var E,
						D,
						H = 0,
						I,
				...
				A.bind("UploadProgress", function(C, D) {
				...
				});A.bind("QueueChanged", n);A.bind("Error", function(C, D) {
					if (D.file) {
					...
					D.status = g.DONE;
					D.loaded = D.size;C.trigger("UploadProgress", D);c(function() {
						s.call(A)
					}, 1)
				});
				if (r.runtimes) {
				...
				function v() {
					...
					if (F) {
						...
						F.init(A, function(G) {
							if (G && G.success) {
								A.features = E;
								A.runtime = F.name;A.trigger("Init", {
									runtime : F.name
								});A.trigger("PostInit");A.refresh()
							} else {
								v()
							}
						})
			...
			start : function() {
				if (t.length && this.state != g.STARTED) {
					this.state = g.STARTED;this.trigger("StateChanged");s.call(this)
				}
			},
			stop : function() {
				if (this.state != g.STOPPED) {
					this.state = g.STOPPED;this.trigger("CancelUpload");this.trigger("StateChanged")
				}
			},
			disableBrowse : function() {
				p = arguments[0] !== b ? arguments[0] : true;this.trigger("DisableBrowse", p)
			},
			getFile : function(w) {
				var v;
			...
			splice : function(x, v) {
				var w;
				w = t.splice(x === b ? 0 : x, v === b ? t.length : v);this.trigger("FilesRemoved", w);this.trigger("QueueChanged");return w
			},
			trigger : function(w) {
