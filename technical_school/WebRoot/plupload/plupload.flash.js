(function(f, b, d, e) {
	...
	d.runtimes.Flash = d.addRuntime("flash", {
		...
		init : function(m, o) {
			...
			l.id = m.id + "_flash_container";d.extend(l.style, {
				position : "absolute",
				top : "0px",
			...
			i.appendChild(l);(function() {
				...
				if (d.ua.ie) {
					q = b.createElement("div");l.appendChild(q);
					q.outerHTML = p;
					q = null
				} else {
					l.innerHTML = p
			...
			j();
			k = l = null;m.bind("Destroy", function(p) {
				var q;
				...
				g[m.id] = true;m.bind("UploadFile", function(s, u) {
					var v = s.settings,
						t = m.settings.resize || {};
					...
					if (u.status != d.FAILED) {
						u.loaded = s.loaded;
						u.size = s.size;t.trigger("UploadProgress", u)
					}
					...
					if (u.chunk == u.chunks - 1) {
						t.status = d.DONE;s.trigger("FileUploaded", t, {
							response : u.text
						})
					}
					...
					for (t = 0; t < v.length; t++) {
						...
						x = d.guid();
						r[x] = u.id;
						r[u.id] = x;w.push(new d.File(x, u.name, u.size))
					}
					if (w.length) {
					...
					if (t && u) {
						d.addClass(t, u);d.addEvent(b.body, "mouseup", function() {
							d.removeClass(t, u)
						}, s.id)
					}
					...
					if (t) {
						...
						v = d.getSize(t);d.extend(b.getElementById(s.id + "_flash_container").style, {
							top : u.y + "px",
							left : u.x + "px",
							width : v.w + "px",
							height : v.h + "px"
